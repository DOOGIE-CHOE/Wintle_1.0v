...
WaveSurfer.create = function(a) {
    ...
    drawBars: function(a, b) {
        ...
        var o = j / f;
        this.waveCc.fillStyle = this.params.waveColor, this.progressCc && (
            this.progressCc.fillStyle = this.params.progressColor), [this.waveCc, this.progressCc].forEach(
            function(b) {
                if (b)
                    for (var c = 0; f > c; c += m) {
                        var d = Math.round(a[Math.floor(c * o)] / n * i);
                        console.log(2);
                        this.fillRect(c + e, i - d + h, k + e, 2 * d);
                    }
            }, this)
    },
    drawWave: function(a, b) {
        ...
        this.waveCc.fillStyle = this.params.waveColor, this.progressCc && (this.progressCc.fillStyle = this.params.progressColor), [this.waveCc, this.progressCc].forEach(
            function(b) {
                if (b) {
                    b.beginPath(),
                        b.moveTo(h, k + j);
    ...
    fillRect: function (x, y, width, height) {
        ...
        for (var i = startCanvas; i < endCanvas; i++) {
            ...
            if (intersection.x1 < intersection.x2) {
                ...
                //         intersection.x2 - intersection.x1,
                //         intersection.y2 - intersection.y1);
                this.roundRect(entry.waveCtx,
                    intersection.x1 - leftOffset,
                    intersection.y1,
                    intersection.x2 - intersection.x1,
                    intersection.y2 - intersection.y1);

                this.roundRect(entry.progressCtx,
                    intersection.x1 - leftOffset,
                    intersection.y1,
                    intersection.x2 - intersection.x1,
                    intersection.y2 - intersection.y1);
    ...
    roundRect: function(ctx, x, y, width, height){
        console.log(width);
        var cornerRadius = width - 1;
        ctx.lineJoin = "round";
